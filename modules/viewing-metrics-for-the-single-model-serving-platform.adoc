...
. For example, you can use the following query to check the number of successful inference requests over a period of time (per model) for models deployed with the vLLM runtime: 
+
`sum(increase(vllm:request_success_total{namespace='${namespace}',model_name='${model_name}'}[${rate_interval}]))`
+
[source,subs="+quotes"]
...
. You can use the following query to check the number of successful inference requests over a period of time (per model) for a model deployed with the TGIS runtime:
+
[source,subs="+quotes"]
